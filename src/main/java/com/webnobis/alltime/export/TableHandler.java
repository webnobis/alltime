...
public class TableHandler {
	...
	public void addEntryTable(List<Entry> entries) {
		...
		Table table = new Table(entryTableColumns.stream().map(EntryTableColumn::getWidthWeight)
				.map(UnitValue::createPointValue).toArray(UnitValue[]::new), true);
	...
	private void addEntryTableHeader(Table table) {
		entryTableColumns.stream().map(EntryTableColumn::getHeader).map(this::headerToCell)
				.forEach(table::addHeaderCell);
	}

	private Cell headerToCell(String text) {
		return new Cell().setBold().setFont(font).setFontSize(headerFontSize)
				.setBackgroundColor(ColorConstants.GRAY, 0.5f).add(new Paragraph(text));
	}

	private void addEntryTableRow(Table table, int row, Entry entry) {
		entryTableColumns.stream().map(column -> {
			switch (column) {
			case DAY:
	...
	private Cell contentToCell(int row, String text) {
		Cell cell = new Cell().setFont(font).setFontSize(cellFontSize)
				.add(new Paragraph(Optional.ofNullable(text).orElse("")));
	...
	private String toString(Map<String, Duration> items) {
		return new TreeMap<>(items).entrySet().stream().map(entry -> {
	...
	private void addTimeAssetsSumTableRows(Table table, TimeAssetsSum sumBefore, TimeAssetsSum sumNow) {
		IntStream.range(0, timeAssetsSumTableRows.size()).boxed().flatMap(i -> {
